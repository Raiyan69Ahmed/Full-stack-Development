...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $sql = "INSERT INTO music_library
                (title, artist, album, genre, year, duration, bpm, key_signature,
                 record_label, producer, spotify_streams, youtube_views, spotify_url, youtube_url)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

        $stmt = getPDO()->prepare($sql);
        $stmt->execute([
            $title, $artist, $album, $genre, $year, $duration, $bpm,
            $key_signature, $record_label, $producer, $spotify, $youtube,
